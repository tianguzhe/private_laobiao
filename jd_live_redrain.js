...
!(async () => {
  ...
  if (!($.st <= nowTs && nowTs < $.ed)) {
    ...
    $.http.get({url: `https://purge.jsdelivr.net/gh/tianguzhe/private_laobiao@master/redrain.json`}).then((resp) => {}).catch((e) => $.log('刷新CDN异常', e));
    let hour = (new Date().getUTCHours() + 8) % 24;
    let redIds = await getRedRainIds();
    if (!redIds) redIds = await getRedRainIds('https://cdn.jsdelivr.net/gh/tianguzhe/private_laobiao@master/redrain.json');
    $.newAcids = [...(redIds || [])];
...
function getRedRainIds(url = "https://raw.githubusercontent.com/tianguzhe/private_laobiao/master/redrain.json") {
